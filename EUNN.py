...
def _eunn_param(hidden_size, capacity=2, fft=False, comp=True):
    ...
    if fft:
        ...
        if comp:
            ...
            cos_list_1 = tf.complex(tf.multiply(
                cos_theta, cos_phi), tf.multiply(cos_theta, sin_phi))
            sin_list_0 = tf.complex(sin_theta, tf.zeros_like(sin_theta))
            sin_list_1 = tf.complex(
                -tf.multiply(sin_theta, cos_phi),
            ...
            if comp:
                cos_list_normal = tf.concat(
                    [tf.slice(cos_list_0, [last], [normal_size]),
                     tf.slice(cos_list_1, [last], [normal_size])], 0)
                sin_list_normal = tf.concat(
                    [tf.slice(sin_list_0, [last], [normal_size]),
                     -tf.slice(sin_list_1, [last], [normal_size])], 0)
                last += normal_size
                ...
                cos_list_extra = tf.concat(
                    [tf.slice(cos_list_0, [last], [extra_size]),
                     tf.complex(
                         ...
                         tf.zeros(
                             [hidden_size - 2 * normal_size
                              - 2 * extra_size])),
                     tf.slice(cos_list_1, [last], [extra_size])], 0)
                sin_list_extra = tf.concat(
                    [tf.slice(sin_list_0, [last], [extra_size]),
                     tf.complex(tf.zeros(
                        ...
                        tf.zeros(
                         [hidden_size - 2 * normal_size - 2 * extra_size])),
                     -tf.slice(sin_list_1, [last], [extra_size])], 0)
                last += extra_size
        ...
        if comp:
            ...
            cos_list_0_re = tf.reshape(tf.concat(
                [cos_theta_0, tf.multiply(cos_theta_0, cos_phi_0)], 2),
                [capacity_a, -1])
            cos_list_0_im = tf.reshape(tf.concat([tf.zeros_like(
                cos_theta_0), tf.multiply(cos_theta_0, sin_phi_0)], 2),
                [capacity_a, -1])
            ...
            sin_list_0_re = tf.reshape(tf.concat(
                [sin_theta_0, - tf.multiply(sin_theta_0, cos_phi_0)], 2),
                [capacity_a, -1])
            sin_list_0_im = tf.reshape(tf.concat([tf.zeros_like(
                sin_theta_0), - tf.multiply(sin_theta_0, sin_phi_0)], 2),
                [capacity_a, -1])
            if hidden_size_a * 2 != hidden_size:
            ...
            cos_list_1_re = tf.reshape(tf.concat(
                [cos_theta_1, tf.multiply(cos_theta_1, cos_phi_1)], 2),
                [capacity_b, -1])
            cos_list_1_re = tf.concat(
                [tf.ones((capacity_b, 1)), cos_list_1_re], 1)
            cos_list_1_im = tf.reshape(tf.concat([tf.zeros_like(
                cos_theta_1), tf.multiply(cos_theta_1, sin_phi_1)], 2),
                [capacity_b, -1])
            ...
            sin_list_1_re = tf.reshape(tf.concat(
                [sin_theta_1, -tf.multiply(sin_theta_1, cos_phi_1)], 2),
                [capacity_b, -1])
            sin_list_1_re = tf.concat(
                [tf.zeros((capacity_b, 1)), sin_list_1_re], 1)
            sin_list_1_im = tf.reshape(tf.concat([tf.zeros_like(
                sin_theta_1), -tf.multiply(sin_theta_1, sin_phi_1)], 2),
                [capacity_b, -1])
        ...
        if capacity_b != capacity_a:
            if comp:
                cos_list_1 = tf.concat([cos_list_1, tf.complex(
                    tf.zeros([1, hidden_size]),
                    tf.zeros([1, hidden_size]))], 0)
                sin_list_1 = tf.concat([sin_list_1, tf.complex(
                    tf.zeros([1, hidden_size]),
                    tf.zeros([1, hidden_size]))], 0)
            else:
                cos_list_1 = tf.concat(
...
class EUNNCell(RNNCell):
    ...
    def __init__(self, hidden_size, capacity=2, fft=False,
                 comp=False, activation=modrelu):
